...
<head>
	...
	<script>
		...
		async function getData() {
			...
			if (token && aposta) {
				console.log('🎮 Jetpack: Usando parâmetros da URL:', { 
					_bet, _player_speed, _missile_speed, _coin_rate, _meta_multiplier 
				});
				return;
			}
			...
			const apiUrl = baseurl ? baseurl + '/jetpack-exclusive/info?token=' + encodeURIComponent(token) : '/demo-game2/jetpack-exclusive/info';
			const data = await fetchApi(apiUrl);

			if (!data || !data.last_balance || !data.last_balance.amount) {
				alert("Você precisa iniciar um jogo");
				// Redirecionar para página de perda
				location.href = "/modal2/jetpack-exclusive/loss/0";
				return;
			}
		...
		async function winGame(valor) {
			...
			const winUrl = baseurl ? baseurl + '/jetpack-exclusive/win' : '/demo-game2/jetpack-exclusive/win';
			...
			location.href = "/modal2/jetpack-exclusive/win/" + (valor || 0);
		}
		...
		async function loseGame(accumuled, bet) {
			...
			const lostUrl = baseurl ? baseurl + '/jetpack-exclusive/lost' : '/demo-game2/jetpack-exclusive/lost';
			...
			console.log('🎮 Jetpack: URL de redirecionamento: /modal2/jetpack-exclusive/loss/' + bet);
			
			// Redirecionar para página de perda (seguindo padrão demosub7)
			console.log('🎮 Jetpack: Executando redirecionamento...');
			location.href = "/modal2/jetpack-exclusive/loss/" + bet;
