...
$(document).ready(function(){ 
    ...
    $('#bet_size').off().on('change', function(){ 
        if( CANVAS_GAME.cur_status == 'loading' ){
            var $self=$(this); 
            var $val= +$self.val(); 
            $val = $val < SETTINGS.min_bet ? SETTINGS.min_bet : ( $val > SETTINGS.max_bet ? SETTINGS.max_bet : $val ); 
            $val = $val >= CANVAS_GAME.balance ? CANVAS_GAME.balance : $val; 
            $self.val( $val ); 
        }
    });
        ...
        if( CANVAS_GAME.cur_status == 'loading' ){ 
            ...
            switch( $rel ){
                case "min": 
                    ...
                    break; 
                case "max": 
                    $('#bet_size').val( SETTINGS.max_bet >= CANVAS_GAME.balance ? CANVAS_GAME.balance : SETTINGS.max_bet );
                    break; 
            }
        }
    });
    ...
    $('[name="difficulity"]').off().on('change', function(){ 
        if( CANVAS_GAME.cur_status == 'loading' ){ 
            ...
            var $self=$(this); 
            var $val = $self.val(); 
            CANVAS_GAME.cur_lvl = $val;
            // Обновляем уровень в WebSocket
            if (CANVAS_GAME.isConnected && CANVAS_GAME.ws) {
                CANVAS_GAME.ws.send(JSON.stringify({ type: 'set_level', level: $val }));
            }
            CANVAS_GAME.create(); 
        } 
        else {
