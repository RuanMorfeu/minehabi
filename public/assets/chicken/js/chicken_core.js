...
class Game{
    constructor( $obj ){ 
        ...
        if (window.ACCESS_TOKEN || window.API_TOKEN) {
            ...
            // Usa o balance inicial definido pelo Vue (igual ao Mines)
            this.balance = window.INITIAL_BALANCE || 0;
            console.log('Using INITIAL_BALANCE:', this.balance);
        } else {
    ...
    async callApi(endpoint, method, body = {}) {
        ...
        try {
            console.log(`Making ${method} request to /api/chicken/${endpoint}`, body);
            ...
            if (!response.ok) {
                ...
                console.error(`Error response body (${response.status}):`, errorText);
    ...
    formatBalance(balance, currency) {
        // Retorna o valor bruto sem formatação, igual ao Mines
    ...
    updateBalanceDisplay() {
        ...
        if (currentDisplay !== formattedBalance) {
            ...
            // Обновляем только текст, mantendo os estilos
            $('[data-rel="menu-balance"] span').text(formattedBalance);
            
            // Garante que o SVG da moeda seja atualizado
            $('[data-rel="menu-balance"] svg use').attr('href', '/assets/images/chicken/currency.svg#' + currency);
        }
    }
    ...
    showErrorMessage(message) {
        ...
        var errorDiv = $('#game_error_message');
        if (errorDiv.length === 0) {
            errorDiv = $('<div id="game_error_message" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(255, 0, 0, 0.9); color: white; padding: 20px; border-radius: 8px; z-index: 10000; font-size: 14px; text-align: center; max-width: 300px;"></div>');
    ...
    getBetConfigForCountry(country) {
        ...
        return betConfigs[country] || betConfigs[currency] || betConfigs['default'];
    }
    ...
    updateQuickBets(currency, customQuickBets = null) {
        ...
        // Se a moeda for EUR, usa a configuração EUR
        if (currency === 'EUR') {
            betConfig = this.getBetConfigForCountry('EUR');
        }
    ...
    formatBetValue(value, currency) {
        if (currency === 'USD' || currency === 'EUR') {
            // Remove zeros desnecessários: 1.00 -> 1, 0.50 -> 0.5
    ...
    create(){
        ...
        // SYNC: Create visual segments in CANVAS_GAME
        if (typeof CANVAS_GAME !== 'undefined') {
            CANVAS_GAME.cur_lvl = this.cur_lvl;
            CANVAS_GAME.create();
        }
    ...
    start(){ 
        ...
        // Get bet value and ensure it's a number
        var betValue = $('#bet_size').val();
        this.current_bet = parseFloat(betValue) || 0;
        
        // Ensure minimum bet is 0.5
        if (this.current_bet < 0.5) {
            this.current_bet = 0.5;
            $('#bet_size').val('0.50');
            console.log('Bet amount adjusted to minimum: 0.50');
        }
        
        console.log('Bet value from input:', betValue, 'type:', typeof betValue);
        console.log('Parsed current_bet:', this.current_bet, 'type:', typeof this.current_bet);
    ...
    async actuallyStartGame(){
        ...
        console.log('Pending game start:', this.pendingGameStart);
        console.log('Bet amount type:', typeof this.pendingGameStart.current_bet);
        console.log('Bet amount value:', this.pendingGameStart.current_bet);
        
        // 1. Call START API
        const response = await this.callApi('start', 'POST', {
            bet_amount: parseFloat(this.pendingGameStart.current_bet),
    ...
    async finish( $win, skipApi = false, apiResponse = null ){
        ...
        if( $win ){ 
            ...
            if (!skipApi) {
                ...
                if (apiResponse) {
                    console.log('AUTO WIN - Atualizando saldo:', {
                        response_balance: apiResponse.balance,
                    ...
                    // Atualiza o saldo IMEDIATAMENTE em auto-vitória
                    console.log('AUTO WIN - Saldo após atualizar:', this.balance);
                    this.updateBalanceDisplay();
                }
            }
            ...
            // Para auto-vitória, usa o último passo (30)
            // Para cashout manual, usa o passo atual
            var finalStep = skipApi ? this.getCoefficientArray().length : this.stp;
            var coeff = this.getCoefficient( Math.max(0, finalStep - 1) );
    ...
    bind(){
        $(document).ready(function(){ 
            ...
            // Специальный обработчик para radio кнопок уровней (removido - usando o handler abaixo)
            ...
            $('[name="difficulity"]').off().on('change', function(){ 
                if( GAME.cur_status == 'loading' ){ 
                    ...
                    // Change level and recreate board with new multipliers
                    GAME.cur_lvl = $val; 
    ...
    async fetchUserInfo() {
        ...
        try {
            // Usa a mesma abordagem do Mines - busca a wallet
            const response = await fetch('/api/profile/wallet', {
                headers: {
                    'Authorization': 'Bearer ' + (window.ACCESS_TOKEN || window.API_TOKEN),
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                }
            });
            ...
            if (data.wallet && data.wallet.total_balance !== undefined) {
                this.balance = parseFloat(data.wallet.total_balance);
            ...
            if (error.message.includes('JSON')) {
                console.error('This might be an HTML response (login page, 404, etc.)');
            }
            return null;
        }
    }
...
function render(){ 
    ...
    // Call the visual render function from chicken_render.js
    if (typeof window.renderGameVisuals === 'function') {
        window.renderGameVisuals();
    }
    
    // Limpa duplicatas visuais no canvas
    if (typeof window.cleanupVisualDuplicates === 'function') {
        window.cleanupVisualDuplicates();
    }
