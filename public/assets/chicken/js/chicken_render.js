...
var IMAGES = "/assets/images/chicken/";
var $fire_big = [ ]; 
for( var $i=0; $i<21; $i++ ){ 
...
class Helpers {
    ...
    image( $obj ){
        ...
        if( $obj.img && $obj.img.complete && $obj.img.naturalHeight !== 0 ){
            this.ctx.drawImage(
                $obj.img,
                $obj.sx,
                $obj.sy,
...
class Flame{
    ...
    update(){ 
        ...
        if( this.alife ){ 
            ...
            if( ( CANVAS_GAME.cur_status == "game" || CANVAS_GAME.cur_status == "loading" ) && CANVAS_GAME.stp != $segment && $start ){
                this.alife = 1; 
...
class Segment {
    ...
    move(){ 
        ...
        if( CANVAS_GAME.segment && this.id == ( CANVAS_GAME.stp - 1 ) && this.x == this.new_x ){
            CANVAS_GAME.moving = 0; 
            CANVAS_GAME.segment = 0; 
    ...
    draw(){
        ...
        if( this.type == "step" ){ 
            ...
            // если там же 
            if( this.id == CANVAS_GAME.stp - 1 ){
                HELPERS.image({
                    img: CANVAS_CHICKEN.alife ? this.bet2 : ( !CANVAS_GAME.win ? this.bet3 : this.bet2 ),
                    sx: 0,
                    sy: 0,
                    sw: 401,
                    sh: 413,
                ...
                if( !CANVAS_CHICKEN.alife ){
                    $draw_text = !CANVAS_GAME.win ? 0 : 1; 
                }
            }
            // если впереди
            if( this.id > CANVAS_GAME.stp ){ 
                this.ctx.save(); 
                this.ctx.globalAlpha = 0.5; 
                HELPERS.image({
                img: this.bet1,
            ...
            if( $draw_text ){ 
                this.ctx.save(); 
                HELPERS.text(
                    SETTINGS.cfs[ CANVAS_GAME.cur_lvl ][ this.id ] + 'x', 
                    this.x + this.w / 2, 
                    this.h - 50 - 270 - 70, 
                    { 
                        shadow: 1, 
            ...
            // trigger 
            if( this.id == ( CANVAS_GAME.stp - 1 ) && CANVAS_GAME.moving_time > 500 ){ 
        ...
        if( this.type == "finish" ){ 
            ...
            if( this.id == ( CANVAS_GAME.stp - 1 ) && CANVAS_GAME.moving_time > 500 ){
                ...
                if( this.fire ){
                    $draw_egg = $lose_egg; 
                    if( CANVAS_GAME.cur_status == "game" ){ CANVAS_GAME.finish(); }
                }
                if( CANVAS_GAME.cur_status == "game" ){ CANVAS_GAME.finish(1); }
            } 
            else {
                if( !CANVAS_CHICKEN.alife ){ 
                    if( !CANVAS_GAME.win ){ 
            ...
            // text 
            if( CANVAS_CHICKEN.alife || ( !CANVAS_CHICKEN.alife && CANVAS_GAME.win ) ){ 
                this.ctx.save(); 
                HELPERS.text(
                    SETTINGS.cfs[ CANVAS_GAME.cur_lvl ][ this.id ] + 'x', 
                    this.x + this.w / 2, 
        ...
        if( this.id == CANVAS_GAME.stp - 1 && this.fire && CANVAS_GAME.moving_time > 600 ){
            HELPERS.image({
                img: $corpse,
                sx: 0,
                sy: 0,
...
class CanvasChicken {
    ...
    move( ){
        if( this.x < this.new_x && CANVAS_GAME.chicken ){ 
            ...
            if( this.x >= this.new_x ){
                this.x = this.new_x;  
                CANVAS_GAME.moving = 0; 
                CANVAS_GAME.chicken = 0; 
                //console.log("STOP FROM CANVAS_CHICKEN");
            }
        } 
    } 
    draw(){ 
...
var CANVAS_CHICKEN = new CanvasChicken({});
CANVAS_CHICKEN.drop(); 
...
class CanvasGame {
    ...
    generateLocalTraps() {
        ...
        // Генерируем трапы на основе коэффициентов
        for (let i = 0; i < cfs.length; i++) {
            // Вероятность трапа зависит от коэффициента (чем выше коэффициент, тем меньше вероятность)
            const trapProbability = Math.max(0.1, 1 / cfs[i]);
            traps.push({
                position: i,
                coefficient: cfs[i],
                isTrap: Math.random() < trapProbability
            });
        }
    ...
    create(){
        ...
        // flame segment - используем локальные трапы
        var $flame_segment;
        if (this.localTraps && this.localTraps.length > 0) {
            // Находим первый трап в локальных данных
            const trap = this.localTraps.find(t => t.isTrap);
            $flame_segment = trap ? trap.position + 1 : Math.ceil( Math.random() * SETTINGS.chance[ this.cur_lvl ][ Math.round( Math.random() * 100  ) > 90 ? 1 : 0 ] );
        } else {
    ...
    finish( $win ){ 
        ...
        window.$rebuild = setTimeout(
            function(){ 
                $('#overlay').hide(); 
                CANVAS_GAME.cur_status = "loading"; 
                $('#win_modal').hide(); 
                CANVAS_GAME.create();  
            }, $win ? 5000 : 3000  
        ); 
        ...
        .then(data => {
            ...
            if (window.parent && window.parent !== window) {
                window.parent.postMessage({
                    ...
                    userId: window.CANVAS_GAME_CONFIG.user_id
                }, '*');
            }
        })
        .catch(error => {
    ...
    make_stp(){ 
        if( this.alife && !this.moving ){ 
            ...
            if( CANVAS_CHICKEN.x < SETTINGS.w / 3 ){ 
                this.chicken = 1;
                this.segment = 0;
                CANVAS_CHICKEN.new_x = CANVAS_CHICKEN.x + SETTINGS.segw; 
            } 
            else { 
    ...
    update(){ 
        ...
        switch( this.cur_status ){
            case 'loading': 
                ...
                $('#close_bet span').html( 0+' '+CANVAS_GAME.currency ).css('display', 'none');
        ...
        if( Math.round( Math.random() * 100 ) > 99 ){ 
            $('#stats span.online').html( window.LOCALIZATION.TEXT_LIVE_WINS_ONLINE + ': '+ Math.round( Math.random() * 10000 ));
            this.random_bet(); 
        }
    } 
} 

var CANVAS_GAME = new CanvasGame({ 
    ctx: $ctx, 
    cur_lvl: 'easy'
});  
...
window.renderGameVisuals = render;

// Função para limpar duplicatas visuais no canvas
window.cleanupVisualDuplicates = function() {
    // Limpa duplicatas de segmentos visuais se houver
    if (SEGMENTS && SEGMENTS.length > 20) {
        console.warn('Found too many segments, cleaning up duplicates');
        SEGMENTS.splice(20); // Mantém apenas os primeiros 20 segmentos
    }
    
    // Garante que não há múltiplas galinhas visuais
    if (CHICKEN && CHICKEN.length > 1) {
        console.warn('Found multiple chicken instances, keeping only one');
        CHICKEN.splice(1); // Mantém apenas a primeira instância
    }
};
...
$(document).ready(function(){ 
    ...
    // Event listeners removed to avoid conflict with chicken_core.js
    // Logic is now handled by GAME instance in chicken_core.js 
