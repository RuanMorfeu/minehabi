...
$(document).ready(function () {
    ...
    if (wallet_balance) {
        let initBalance = parseFloat(wallet_balance).toFixed(2).replace('.', ',');
        $("#wallet_balance").text(currency_symbol + " " + initBalance); // Show Wallet Balance
        $("#header_wallet_balance").text(currency_symbol + " " + initBalance); // Show Header Wallet Balance
    } else {
        $("#wallet_balance").text(currency_symbol + " 0,00"); // Show Wallet Balance
...
function info_data(intialData) {
    ...
    if (intialData.wallet_balance !== undefined && intialData.wallet_balance !== null) {
        let formattedBalance = parseFloat(intialData.wallet_balance).toFixed(2).replace('.', ',');
        // Garante que o currency_symbol tenha um espaço se necessário, ou ajusta conforme o padrão visual
        $("#wallet_balance").text(currency_symbol + " " + formattedBalance);
        $("#header_wallet_balance").text(currency_symbol + " " + formattedBalance);
    }
        ...
        success: function (result) {
            if (result.isSuccess) {
                if (result.data.wallet_balance != '' && result.data.wallet_balance != NaN && result.data.wallet_balance != 'NaN') {
                    let newBalance = parseFloat(result.data.wallet_balance).toFixed(2).replace('.', ',');
                    $("#wallet_balance").text(currency_symbol + " " + newBalance);
                    $("#header_wallet_balance").text(currency_symbol + " " + newBalance); // Show Header Wallet Balance
                } else {
...
$(window).focus(function () {
    ...
    if (promise !== undefined) {
        promise.then(_ => {
            ...
            // Autoplay was prevented.
            console.log("Audio autoplay prevented by browser policy on focus");
        });
    }
