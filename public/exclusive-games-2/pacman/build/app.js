...
class Ghost {
  ...
  setSpriteSheet(name, direction, mode) {
    // Only enable emotion sprites for Blinky (red ghost) - others caused invisibility
    let emotion = "";
   ...
   * Adjusted based on difficulty and meta progress
   * @param {({x: number, y: number})} pacmanGridPosition
   * @returns {({x: number, y: number})}
   */
  determinePinkyTarget(pacmanGridPosition) {
    // Get meta progress for dynamic difficulty
    const progress = window.gameCoordinator ? window.gameCoordinator.getMetaProgress() : 0;
    const difficulty = window._game_difficulty || 1;
    
    // Base distance is 4 tiles, but increases with difficulty and progress
    let targetDistance = 4;
    
    // Increase targeting distance based on difficulty and progress
    if (difficulty === 2) {
      targetDistance += Math.floor(progress * 2); // Up to +2 tiles
    } else if (difficulty === 3) {
      targetDistance += Math.floor(progress * 4); // Up to +4 tiles
    }
    
    // Cap maximum distance
    targetDistance = Math.min(targetDistance, 8);
    
    return this.getPositionInFrontOfPacman(pacmanGridPosition, targetDistance);
  }
   ...
   * bottom-left corner when they are close together
   * Adjusted based on difficulty and meta progress
  ...
  determineClydeTarget(gridPosition, pacmanGridPosition) {
    ...
    // Base flee distance is 8, but decreases with difficulty and progress
    let fleeDistance = 8;
    
    // Decrease flee distance based on difficulty and progress (more aggressive)
    if (difficulty === 2) {
      fleeDistance -= Math.floor(progress * 2); // Down to 6 tiles
    } else if (difficulty === 3) {
      fleeDistance -= Math.floor(progress * 4); // Down to 4 tiles
    }
    
    // Minimum flee distance
    fleeDistance = Math.max(fleeDistance, 3);
    
    return distance > fleeDistance ? pacmanGridPosition : { x: 0, y: 30 };
  }
  ...
  draw(interp) {
    ...
    this.animationTarget.style.visibility = this.display ? "visible" : "hidden";
...
class GameCoordinator {
  constructor() {
    // Store reference globally for ghost access
  ...
  reset() {
    ...
    // Initialize global window.score for difficulty system
    window.score = 0;
    
    // Initialize difficulty tracking variables
    this.blinkyRage1 = false;
    this.blinkyRage2 = false;
    this.extraGhostsReleased = false;
    ...
    this.pointsDisplay.innerHTML = "€ 0,00";
  ...
  handlePauseKey() {
    if (this.allowPause) {
      ...
      this.pausedText.innerText =
        "Voce ganhou €" + `${formatCurrency(window.parent.wA)}`;
   ...
   */
  awardPoints(e) {
    console.log('💰 awardPoints chamado:', e.detail.points, 'pontos totais antes:', this.points);
    ...
    // Update global window.score for difficulty system
    window.score = this.points;
    
    console.log('💰 Pontos atualizados:', this.points, 'window.score:', window.score);
    
    // Check Blinky rage immediately after score update
    this.checkBlinkyRage();
    ...
    this.pointsDisplay.innerText = `€ ${formatCurrency(this.points)}`;
  ...
  powerUp() {
    ...
    // Calculate power duration based on difficulty and meta progress
    const baseDuration = Math.max((7 - this.level) * 1000, 0);
    const progress = this.getMetaProgress();
    const difficulty = this.getDifficultyLevel();
    
    // Reduce power duration based on difficulty and progress
    let durationMultiplier = 1.0;
    
    // OVER META MODE - Power pellets last almost nothing
    // Skip over meta for influencers or demo agents
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      // Adjust power pellet duration based on difficulty level
      if (difficulty === 1) {
        durationMultiplier = 0.20; // Easy: 20% duration
      } else if (difficulty === 2) {
        durationMultiplier = 0.15; // Medium: 15% duration (was hard level)
      } else if (difficulty === 3) {
        durationMultiplier = 0.08; // Hard: 8% duration (even more punishing)
      }
      console.log('🚨 OVER META: Power pellet duration severely reduced!', durationMultiplier.toFixed(2) + 'x', 'Difficulty:', difficulty);
    } else if (difficulty === 2) {
      durationMultiplier = 0.9 - (progress * 0.2); // 90% to 70%
    } else if (difficulty === 3) {
      durationMultiplier = 0.5 - (progress * 0.1); // 50% to 40%
    }
    
    // Minimum duration adjusted for over meta based on difficulty
    // Skip over meta for influencers or demo agents
    let minDuration = 0.3; // Default minimum
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      if (difficulty === 1) {
        minDuration = 0.25; // Easy: reduced minimum (from 0.4)
      } else if (difficulty === 2) {
        minDuration = 0.3; // Medium: standard minimum
      } else if (difficulty === 3) {
        minDuration = 0.2; // Hard: very low minimum
      }
    }
    durationMultiplier = Math.max(durationMultiplier, minDuration);
    
    // Base duration adjusted for over meta difficulty scaling
    let basePowerDuration = 1500; // Default
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      if (difficulty === 1) {
        basePowerDuration = 700; // Easy: 0.7 seconds (reduced from 1.0)
      } else if (difficulty === 2) {
        basePowerDuration = 800; // Medium: 0.8 seconds
      } else if (difficulty === 3) {
        basePowerDuration = 600; // Hard: 0.6 seconds
      }
    }
    
    const powerDuration = Math.max(baseDuration * durationMultiplier, basePowerDuration);
    
    console.log('⚡ Power Pellet Duration:', {
      baseDuration: baseDuration + 'ms',
      durationMultiplier: durationMultiplier.toFixed(2),
      finalDuration: powerDuration + 'ms',
  ...
  /**
   * Get current difficulty level from global variable
   * @returns {number} Difficulty level (1-3)
   */
  getDifficultyLevel() {
    const difficulty = window._game_difficulty || 1;
    console.log('🎮 Difficulty Detection:', {
      window_game_difficulty: window._game_difficulty,
      resolved_difficulty: difficulty,
      difficulty_name: difficulty === 1 ? 'Fácil' : difficulty === 2 ? 'Médio' : 'Difícil'
    });
    return difficulty;
  }

  /**
   * Get current meta progress percentage
   * @returns {number} Progress percentage (0-1)
   */
  getMetaProgress() {
    // Get current score and meta target from global variables
    const currentScore = window.score || 0;
    const metaTarget = (window._meta_multiplier || 1) * (window._bet || 1);
    const progress = metaTarget <= 0 ? 0 : currentScore / metaTarget;
    
    console.log('🚨 DEBUG META PROGRESS:', {
      'window.score': window.score,
      'window._meta_multiplier': window._meta_multiplier,
      'window._bet': window._bet,
      'window._is_demo_agent': window._is_demo_agent,
      'window._disable_over_meta': window._disable_over_meta,
      'currentScore': currentScore,
      'metaTarget': metaTarget,
      'progress': progress,
      'progressPercent': (progress * 100).toFixed(1) + '%',
      'isOverMeta (>1.05)': progress > 1.05,
      'isInfluencer': !!window._disable_over_meta,
      'overMetaDisabled': progress > 1.05 && (window._is_demo_agent || window._disable_over_meta),
      'overMetaBy': progress > 1.05 ? ((progress - 1.05) * 100).toFixed(1) + '%' : 'N/A',
      'calculation': currentScore + ' / ' + metaTarget + ' = ' + progress
    });
    
    // Alert if over meta should be active but might not be working
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      console.log('🚨🚨🚨 OVER META SHOULD BE ACTIVE! Progress:', (progress * 100).toFixed(1) + '%');
    } else if (progress > 1.05 && window._disable_over_meta) {
      console.log('🎯🎯🎯 INFLUENCER MODE: Over meta DISABLED at', (progress * 100).toFixed(1) + '% - User has influencer status enabled in admin');
    } else if (progress > 1.05 && window._is_demo_agent) {
      console.log('🎮🎮🎮 DEMO MODE: Over meta DISABLED at', (progress * 100).toFixed(1) + '% - Demo/unauthenticated user');
    }
  ...
  /**
   * Check if Blinky should rage based on meta progress
   */
  checkBlinkyRage() {
    const progress = this.getMetaProgress();
    const difficulty = this.getDifficultyLevel();
    
    console.log('🔴 Blinky Rage Check:', {
      progress: progress,
      progressPercent: (progress * 100).toFixed(1) + '%',
      difficulty: difficulty,
      currentScore: window.score,
      metaTarget: (window._meta_multiplier || 1) * (window._bet || 1),
      blinkyRage1: this.blinkyRage1,
      blinkyRage2: this.blinkyRage2
    });
    
    // Calculate rage thresholds based on meta progress - MUITO MAIS CEDO!
    let rageThreshold1, rageThreshold2;
    
    if (difficulty === 1) { // Easy
      rageThreshold1 = 0.5; // 50% of meta
      rageThreshold2 = 0.8; // 80% of meta
    } else if (difficulty === 2) { // Medium
      rageThreshold1 = 0.3; // 30% of meta
      rageThreshold2 = 0.5; // 50% of meta
    } else { // Hard
      rageThreshold1 = 0.02; // 2% of meta
      rageThreshold2 = 0.1; // 10% of meta
    }
    
    console.log('🎯 Rage Thresholds:', {
      threshold1: (rageThreshold1 * 100).toFixed(1) + '%',
      threshold2: (rageThreshold2 * 100).toFixed(1) + '%'
    });
    
    // Check if we've crossed a rage threshold
    if (!this.blinkyRage1 && progress >= rageThreshold1) {
      this.blinkyRage1 = true;
      console.log('🔥 BLINKY RAGE 1 ATIVADO!');
      this.speedUpBlinky();
    } else if (!this.blinkyRage2 && progress >= rageThreshold2) {
      this.blinkyRage2 = true;
      console.log('🔥🔥 BLINKY RAGE 2 ATIVADO!');
      this.speedUpBlinky();
    }
  }

  /**
   * Speed up Blinky ghost when rage is triggered
   */
  speedUpBlinky() {
    const blinky = this.ghosts.find(ghost => ghost.name === 'blinky');
    if (blinky) {
      const progress = this.getMetaProgress();
      
      // OVER META MODE - Blinky becomes extremely fast
      // Skip over meta for influencers or demo agents
      if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
        blinky.cruiseElroy = true;
        
        // Increase Blinky speed based on difficulty level
        const difficulty = this.getDifficultyLevel();
        let speedMultiplier = 1.5; // Base over meta speed
        
        if (difficulty === 2) {
          speedMultiplier = 1.8; // Medium difficulty
        } else if (difficulty === 3) {
          speedMultiplier = 2.2; // Hard difficulty - very fast
        }
        
        blinky.defaultSpeed = blinky.fastSpeed * speedMultiplier;
        console.log('🚨 OVER META: Blinky speed increased', speedMultiplier.toFixed(1) + 'x!', 'Difficulty:', difficulty);
      } else {
        blinky.speedUp();
        console.log('⚡ Blinky speed increased!');
      }
    }
  }

  /**
   * Get difficulty speed multiplier based on meta progress
   * @param {number} progress - Meta progress (0-1)
   * @returns {number} Speed multiplier
   */
  getDifficultySpeedMultiplier(progress) {
    const difficulty = this.getDifficultyLevel();
    const baseMultiplier = 1.0;
    
    // OVER META MODE - Extremely difficult to force quick finish
    // Skip over meta for influencers or demo agents
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      // Base over meta multiplier increased significantly
      let overMetaMultiplier = 4.5; // Increased from 3.5x to 4.5x
      
      // Additional multiplier based on difficulty level
      if (difficulty === 2) {
        overMetaMultiplier = 5.0; // Medium difficulty over meta
      } else if (difficulty === 3) {
        overMetaMultiplier = 6.0; // Hard difficulty over meta - extremely punishing
      }
      
      console.log('🚨 OVER META MODE ACTIVATED! Speed multiplier:', overMetaMultiplier.toFixed(2), 'Difficulty:', difficulty);
      return overMetaMultiplier;
    }
    
    // Adjusted multipliers with higher values for hard difficulty
    if (progress < 0.2) {
      return baseMultiplier + (difficulty * 0.05); // Adjusted for max 1.65x
    } else if (progress < 0.4) {
      return baseMultiplier + (difficulty * 0.10); // Adjusted for max 1.65x
    } else if (progress < 0.7) {
      return baseMultiplier + (difficulty * 0.15); // Adjusted for max 1.65x
    } else {
      return baseMultiplier + (difficulty * 0.217); // Adjusted for max 1.65x (3 * 0.217 = 0.65)
    }
  }


  /**
   * Get difficulty aggressiveness multiplier
   * @returns {number} Aggressiveness multiplier
   */
  getDifficultyAggressiveness() {
    const difficulty = this.getDifficultyLevel();
    const progress = this.getMetaProgress();
    
    // OVER META MODE - Extreme aggressiveness
    // Skip over meta for influencers or demo agents
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      console.log('🚨 OVER META: Maximum aggressiveness activated!');
      return 10.0; // Extremely aggressive
    }
    
    // Adjusted multipliers: Easy=1.0x, Medium=1.5x, Hard=5.0x
    if (difficulty === 1) return 1.0;
    if (difficulty === 2) return 1.5;
    return 5.0; // Hard difficulty
  }

  /**
   * Check if extra ghosts should be released early based on meta progress
   * @param {number} progress - Meta progress (0-1)
   * @returns {boolean} Should release extra ghosts
   */
  shouldReleaseExtraGhosts(progress) {
    const difficulty = this.getDifficultyLevel();
    
    // OVER META MODE - Release all ghosts immediately
    // Skip over meta for influencers or demo agents
    if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
      console.log('🚨 OVER META: All ghosts released immediately!');
      return true;
    }
    
    if (difficulty === 1) return progress >= 0.25; // Easy: 25%
    if (difficulty === 2) return progress >= 0.15; // Medium: 15%
    return progress >= 0.1; // Hard: 10%
  }

  /**
   * Apply dynamic difficulty adjustments to all ghosts
   */
  applyDifficultyAdjustments() {
    const progress = this.getMetaProgress();
    const speedMultiplier = this.getDifficultySpeedMultiplier(progress);
    const shouldRelease = this.shouldReleaseExtraGhosts(progress);
    
    console.log('🎯 Applying Difficulty Adjustments:', {
      progress: (progress * 100).toFixed(1) + '%',
      speedMultiplier: speedMultiplier.toFixed(2),
      shouldReleaseExtra: shouldRelease,
      difficulty: this.getDifficultyLevel()
    });

    // Apply speed multiplier to all ghosts
    this.ghosts.forEach(ghost => {
      if (ghost.originalSlowSpeed === undefined) {
        // Store original speeds first time
        ghost.originalSlowSpeed = ghost.slowSpeed;
        ghost.originalMediumSpeed = ghost.mediumSpeed;
        ghost.originalFastSpeed = ghost.fastSpeed;
      }
      
      // Apply dynamic speed multiplier
      ghost.slowSpeed = ghost.originalSlowSpeed * speedMultiplier;
      ghost.mediumSpeed = ghost.originalMediumSpeed * speedMultiplier;
      ghost.fastSpeed = ghost.originalFastSpeed * speedMultiplier;
      
      // Update current default speed
      if (ghost.defaultSpeed === ghost.originalSlowSpeed) {
        ghost.defaultSpeed = ghost.slowSpeed;
      } else if (ghost.defaultSpeed === ghost.originalMediumSpeed) {
        ghost.defaultSpeed = ghost.mediumSpeed;
      } else if (ghost.defaultSpeed === ghost.originalFastSpeed) {
        ghost.defaultSpeed = ghost.fastSpeed;
      }
      
      // OVER META MODE - Force all ghosts to chase mode permanently
      // Skip over meta for influencers or demo agents
      if (progress > 1.05 && !window._is_demo_agent && !window._disable_over_meta) {
        ghost.mode = "chase";
        ghost.defaultMode = "chase";
        ghost.cruiseElroy = true; // Make all ghosts act like Blinky in cruise elroy
        console.log('🚨 OVER META: Ghost', ghost.name, 'forced to permanent chase mode!');
      }
    });

    // Release extra ghosts early if conditions are met
    if (shouldRelease && !this.extraGhostsReleased) {
      this.releaseExtraGhosts();
      this.extraGhostsReleased = true;
      console.log('👻 Extra ghosts released early due to difficulty!');
    }
  }

  /**
   * Release extra ghosts from ghost house
   */
  releaseExtraGhosts() {
    this.ghosts.forEach(ghost => {
      if (ghost.idleMode === 'idle' && ghost.name !== 'blinky') {
        ghost.endIdleMode();
        console.log(`👻 ${ghost.name} released from ghost house`);
      }
    });
  }
}
...
class CharacterUtil {
  ...
  checkForStutter(position, oldPosition) {
    let stutter = false;
    const threshold = 15; // Increased threshold to handle dynamic speed changes
