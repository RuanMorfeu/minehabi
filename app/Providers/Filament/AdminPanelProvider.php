...
use App\Filament\Admin\Pages\BettingStats;
...
use App\Filament\Admin\Pages\LogsRoundsFreePage;
use App\Filament\Admin\Pages\RoundsFreePage;
use App\Filament\Admin\Pages\SecurityNifPage;
use App\Filament\Admin\Pages\SecurityPage;
...
use App\Filament\Admin\Widgets\KycStatsWidget;
                ...
                return $builder->groups([
                    ...
                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Meus Jogos')
                            ->items([
                                ...
                                ...GameExclusive2Resource::getNavigationItems(),
                                // ...GameSpinsResource::getNavigationItems(),
                                ...
                                NavigationItem::make('games-key')
                                    ...
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),
                                NavigationItem::make('rounds-free')
                                    ->icon('heroicon-o-cpu-chip')
                                    ->label(fn (): string => 'Rodadas Grátis')
                                    ->url(fn (): string => RoundsFreePage::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.pages.rounds-free')),
                                NavigationItem::make('logs-rounds-free')
                                    ->icon('heroicon-o-cpu-chip')
                                    ->label(fn (): string => 'Logs Rodadas Grátis')
                                    ->url(fn (): string => LogsRoundsFreePage::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.pages.logs-rounds-free')),
                    ...
                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Marketing')
                            ->items([
                                ...FacebookAdsResource::getNavigationItems(),

                                NavigationItem::make('facebook-pixel-settings')
                                    ->icon('heroicon-o-share')
                                    ->label(fn (): string => 'Facebook Pixel')
                                    ->url(fn (): string => \App\Filament\Admin\Pages\FacebookPixelSettings::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.admin.pages.facebook-pixel-settings'))
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),
                            ])
                        : NavigationGroup::make(),

                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Verificações KYC')
                            ->items([
                                ...UserAccountResource::getNavigationItems(),
                            ])
                        : NavigationGroup::make(),

                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Popups')
                            ->items([
                                ...AuthPopupResource::getNavigationItems(),

                                NavigationItem::make('sms-settings')
                                    ->icon('heroicon-o-chat-bubble-left-right')
                                    ->label(fn (): string => 'Configurações de SMS')
                                    ->url(fn (): string => SmsSettingResource::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.admin.resources.sms-settings.index'))
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),

                                // Item de navegação para configurações antigas de bônus removido

                                ...\App\Filament\Admin\Resources\InfluencerBonusResource::getNavigationItems(),
                            ])
                        : NavigationGroup::make(),
                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Administração')
                            ->items([
                                ...
                                NavigationItem::make('betting-stats')
                                    ->icon('heroicon-o-chart-bar')
                                    ->label(fn (): string => 'Estatísticas de Apostas')
                                    ->url(fn (): string => BettingStats::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.admin.pages.betting-stats'))
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),
                            ])
                        : NavigationGroup::make(),
                    ...
                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Segurança')
                            ->items([
                                NavigationItem::make('security-page')
                                    ->icon('heroicon-o-shield-check')
                                    ->label(fn (): string => 'Telefones Duplicados')
                                    ->url(fn (): string => SecurityPage::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.admin.pages.security-page'))
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),
                                NavigationItem::make('security-nif-page')
                                    ->icon('heroicon-o-identification')
                                    ->label(fn (): string => 'NIFs Duplicados')
                                    ->url(fn (): string => SecurityNifPage::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.admin.pages.security-nif-page'))
                                    ->visible(fn (): bool => auth()->user()->hasRole('admin')),
