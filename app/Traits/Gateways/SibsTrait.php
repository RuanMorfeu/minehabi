...
trait SibsTrait
{
    public static function finalizePaymentSibs($idTransaction): bool
    {
        ...
        if (! empty($transaction)) {
            ...
            if (! empty($wallet)) {
                ...
                \Log::info('[INFLUENCER-DEBUG] SibsTrait: Checking for influencer bonus.', [
                ...
                \Log::info('[INFLUENCER-DEBUG] SibsTrait: Extracted influencer code.', ['influencer_code' => $influencerCode]);

                // Prioritize Influencer Bonus
                if ($transaction->accept_bonus && ! empty($influencerCode)) {
                    \Log::info('[INFLUENCER-DEBUG] SibsTrait: Main conditions met (accepts bonus and has code), checking details.');
                    ...
                    \Log::info('[INFLUENCER-DEBUG] SibsTrait: Buscando bônus de influencer no banco de dados', [
                        'code' => $influencerCode,
                    ...
                    if ($influencerBonus) {
                        ...
                        if ($isOneTimeUse) {
                            ...
                            \Log::info('[INFLUENCER-DEBUG] SibsTrait: Verificando se bônus já foi resgatado', [
                        ...
                        \Log::info('[INFLUENCER-DEBUG] SibsTrait: Verificando requisitos mínimos', [
                        ...
                        if ($meetsMinDepositRequirement && ! $alreadyRedeemed) {
                            \Log::info('[INFLUENCER-DEBUG] SibsTrait: APPLYING INFLUENCER BONUS.');
                            ...
                            if ($maxBonus > 0 && $calculatedBonus > $maxBonus) {
                                $bonus = $maxBonus;
                                \Log::info('[INFLUENCER-DEBUG] SibsTrait: Bonus limited by max value.', [
                                ...
                                \Log::info('[INFLUENCER-DEBUG] SibsTrait: Bonus calculated without limit.', [
                                    'calculated_bonus' => $calculatedBonus,
                                    'bonus_percentage' => $influencerBonus->bonus_percentage,
                                ]);
                            }
                            ...
                            if ($isOneTimeUse) {
                                try {
                                    ...
                                    \Log::info('[INFLUENCER-DEBUG] SibsTrait: Resgate de bônus de uso único registrado', [
                                        'redemption_id' => $redemption->id,
                                        'user_id' => $transaction->user_id,
                                        'bonus_id' => $influencerBonus->id,
                                    ]);
                                } catch (\Exception $e) {
                                    \Log::error('[INFLUENCER-DEBUG] SibsTrait: Erro ao registrar resgate de bônus', [
                                        'error' => $e->getMessage(),
                                        'user_id' => $transaction->user_id,
                                        'bonus_id' => $influencerBonus->id,
                                    ]);
                            ...
                            \Log::info('[INFLUENCER-DEBUG] SibsTrait: Influencer bonus applied successfully.', [
                                'bonus_amount' => $bonus,
                            ...
                            \Log::info('[INFLUENCER-DEBUG] SibsTrait: Influencer bonus requirements not met.', [
                        ...
                        \Log::info('[INFLUENCER-DEBUG] SibsTrait: No valid influencer bonus found for code.', [
                            'code' => $influencerCode,
                        ]);
                    }
                } else {
                    \Log::info('[INFLUENCER-DEBUG] SibsTrait: Main conditions not met (accept_bonus is false or influencerCode is empty).');
                }
