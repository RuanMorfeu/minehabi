...
class Core
{
                ...
                if ($type === 'win') {
                    ...
                    if (in_array($changeBonus, ['balance', 'balance_withdrawal'])) {
                        if (empty($wallet->balance_deposit_rollover) || $wallet->balance_deposit_rollover <= 0) {
                            ...
                            if ($wallet->balance_deposit_rollover >= $win) {
                                $wallet->decrement('balance_deposit_rollover', $win); // / reduzindo o valor
    ...
    {
        Log::debug("Chegou no generateGameHistory {$userId} {$type} - $tx");
        ...
        // / pagando ganhos para influencers (is_demo_agent = true)
        if ($user->is_demo_agent == true && $changeBonus != 'source' && $type == 'win' && $win > 0) {
            $wallet = Wallet::where('user_id', $userId)->first();
            if (! empty($wallet)) {
                // Adicionar ganho diretamente na carteira de saque para influencers
                $wallet->increment('balance_withdrawal', $win);

                Log::info('Ganho creditado diretamente para influencer via Core', [
                    'user_id' => $userId,
                    'email' => $user->email,
                    'win_amount' => $win,
                    'transaction_id' => $tx,
    ...
    public static function generateGameHistory_trevo($userId, $type, $win, $bet, $changeBonus, $tx)
    {
        Log::debug("Chegou no generateGameHistory {$userId} {$type} - $tx");
    ...
    private static function PayLoss($user, $affiliate, $bet, $changeBonus)
    {
        ...
        Log::debug("Chegou no payloss {$user->id} {$bet}");
        ...
        if (! empty($affiliate)) {
            ...
            if (! empty($affHistoryRevshare) && in_array($changeBonus, ['balance', 'balance_withdrawal'])) {
                ...
                Log::debug('SALDO DO AFILIADO: '.$affiliate->wallet->refer_rewards);
    ...
    {
        Log::debug("Chegou no PayWin {$amount}");
            ...
            if ($type == 'win' && ! empty($user->inviter) && in_array($changeBonus, ['balance', 'balance_withdrawal'])) {
                if (! empty($affiliate)) {
                    ...
                    if (! empty($affHistoryRevshare)) {
                        ...
                        Log::debug('SALDO DO AFILIADO: '.$wallet->refer_rewards);
     ...
     * @param  $value
     */
    public static function formatCurrencyByRegion($amount, $currency = 'EUR'): mixed
    {
    ...
    public static function DecToken($token)
    {
        ...
        if (is_numeric($json)) {
            ...
            return ['status' => false, 'messase' => 'invalid token'];
        }
    }
