<template>
                                ...
                                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400  border dark:border-gray-600">
                                        <tr>
                                            ...
                                            <th v-if="false" scope="col" class="px-6 py-3">
    {{ $t('Type') }}
</th>
                                            <th scope="col" class="px-6 py-3">
                                                {{ $t('Value') }}
                                    ...
                                    <tbody>
                                        <tr v-for="(withdraw, index) in sortedWithdraws" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            <td class="px-6 py-4">
    ...
    <span class="uppercase">{{ withdraw.type }}</span>
                                            ...
                                            <td class="px-6 py-4">
                                                <span v-if="withdraw.status === 1" class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $t('Concluded') }}</span>
                        ...
                        <div v-if="withdraws?.meta" class="mt-4">
                            <CustomPagination
                                :pagination="withdraws.meta"
                                @page-change="onWithdrawPageChange"
                            />
                        </div>
                                ...
                                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        <tr v-for="(deposit, index) in deposits.data" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="px-6 py-4">
                                                <span v-if="deposit.status === 1" class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $t('Concluded') }}</span>
                        ...
                        <div v-if="deposits?.meta" class="mt-4">
                            <CustomPagination
                                :pagination="deposits.meta"
                                @page-change="onDepositPageChange"
                            />
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        sortedWithdraws() {
            if (!this.withdraws?.data) return [];
            return [...this.withdraws.data].sort((a, b) => {
    ...
    methods: {
        ...
        getWithdraws: function() {
            ...
            HttpApi.get(`wallet/withdraw?page=${this.withdrawPage}`)
                .then(response => {
        ...
        getDeposits: function() {
            ...
            HttpApi.get(`wallet/deposit?page=${this.depositPage}`)
                .then(response => {
        ...
        onWithdrawPageChange(page) {
            this.withdrawPage = page;
            this.getWithdraws();
        },
        onDepositPageChange(page) {
