...
<body class="dark-bg-main">
    ...
    <div class="load-txt">
        <div class="loading-game-1">
            <div class="center-loading text-white text-center">
                ...
                <div class="secondary-font f-40 mt-2 waiting-text"></div>
    ...
    {{-- Custom Header: Only showing bottom part (Balance) and hiding Top Blue Nav --}}
    <div class="header-bottom" style="margin-top: 0px !important;">
        <div class="header-left">
            {{-- Logo removido para evitar duplicidade no layout do cassino --}}
        </div>
        <div class="header-right d-flex align-items-center">
            <div class="wallet-balance h-26">
                <span id="wallet_balance">R$ {{ number_format((auth()->user()?->wallet?->balance ?? 0) + (auth()->user()?->wallet?->balance_bonus ?? 0) + (auth()->user()?->wallet?->balance_withdrawal ?? 0), 2, ',', '.') }}</span>
            </div>
        </div>
    </div>

    {{-- Hidden elements required by JS (originally in header-top) --}}
    <div style="display:none;">
        <span id="header_wallet_balance">R$ {{ number_format((auth()->user()?->wallet?->balance ?? 0) + (auth()->user()?->wallet?->balance_bonus ?? 0) + (auth()->user()?->wallet?->balance_withdrawal ?? 0), 2, ',', '.') }}</span>
        <img src="{{ auth()->user()?->avatar ? asset('storage/'.auth()->user()->avatar) : asset('images/default-avatar.png') }}" id="avatar_img">
        <div id="username">{{ auth()->user()?->id ?? 'Guest' }}</div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
            <div class="modal-content">
                ...
                <audio id="sound_Audio">
                    <source src="{{ asset('plane-crash.mp3') }}" type="audio/mpeg">
                </audio>
                <audio id="background_Audio">
                    <source src="{{ asset('background.mp3') }}" type="audio/mpeg">
                </audio>
                <audio id="fly_plane_audio">
                    <source src="{{ asset('game-start.mp3') }}" type="audio/mpeg">
                </audio>
                <audio id="cash_out_audio">
                    <source src="{{ asset('cashout.mp3') }}" type="audio/mpeg">
                </audio>
                <audio id="cash_out_audio_2">
                    <source src="{{ asset('cashout_2.mp3') }}" type="audio/mpeg">
                </audio>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        var currency_id = '{{ \App\Models\Setting::first()->currency_code ?? "BRL" }}';
        var currency_symbol = '{{ \App\Models\Setting::first()->prefix ?? "R$" }}';
        var wallet_balance = '{{ (auth()->user()?->wallet?->balance ?? 0) + (auth()->user()?->wallet?->balance_bonus ?? 0) + (auth()->user()?->wallet?->balance_withdrawal ?? 0) }}';
        var profile_image = '1';
        var member_id = '{{ auth()->id() ?? 0 }}';
        var min_bet_amount = parseFloat('1.00');
        var max_bet_amount = parseFloat('1000.00');
    ...
    <script>
        // Configuração Global do AJAX para incluir o Token JWT
        $.ajaxSetup({
            headers: {
            ...
            beforeSend: function(xhr) {
                ...
                if (token) {
                    ...
                    console.warn('Aviator: Token JWT não encontrado no localStorage');
                }
            }
        });

        // Função global para atualizar o saldo
        window.updateAviatorBalance = function(balance) {
            if (balance !== undefined && balance !== null) {
                let formatted = parseFloat(balance).toFixed(2).replace('.', ',');
                let text = currency_symbol + " " + formatted;
                $("#wallet_balance").text(text);
                $("#header_wallet_balance").text(text);
                console.log('Aviator: Saldo atualizado para ' + text);
            }
        };
    </script>
    <script src="{{ asset('user/aviatorold.js') }}?v={{ time() }}"></script>
    <script src="{{ asset('user/aviatorbyapp.js') }}?v={{ time() }}"></script>

        
        <script>
        $(document).ready(function(){
        
        console.log('Aviator: Iniciando jogo...');
        ...
        $.ajax({
            ...
            success: function(intialData) {
                console.log('Aviator: Dados iniciais recebidos', intialData);
                info_data(intialData);
                
                // Força a atualização do saldo com o valor recebido do backend
                if (intialData.wallet_balance !== undefined) {
                    window.updateAviatorBalance(intialData.wallet_balance);
                }
            },
            error: function(xhr, status, error) {
                console.error('Aviator: Erro ao buscar dados iniciais', error);
            }
        });
    });
    $(".load-txt").hide();
